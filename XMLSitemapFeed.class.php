...
class XMLSitemapFeed {
	...
	*/
	...
	private function post_types() {
		$options = get_option( 'XMLSitemapFeed_sitemaps' );
		if (empty($options))
			$this->post_types = $this->defaults('post_types');
...
*/
	...
	*/
	...
	public function privacy_settings_section() {
		echo '<p>'.__('These settings control the XML Sitemap generation.','xml-sitemap-feed').'</p>';
	}
	...
	private $privacy_setting_array = array(
						...
						//),
						'robots' => array(
							'title' => 'Rules to append to the <a href="/robots.txt" target="_blank">robots.txt</a> generated by WordPress.',
							'label' => 'Additional robots.txt rules',//_('','xml-sitemap-feed'),
							'type' => 'textarea', // select, function, checkbox, text, number, hidden...
							//'function' => 'functionname',
							//'options' => array(''=>'',''=>''),
							'default' => '',
							'class' => 'large-text', // used for text fields
							'description' => 'Warning: Only set rules here when you know what you are doing, otherwise you might break access to your site.<br />Note: These rules will not have effect when you are using a static robots.txt file.',
							'sanitize_callback' => array(__CLASS__,'strip'), //'functionname'
						),
						//'option2' => array(
						//	'label' => 'Test 2',
	...
	function settings_fields($settings){
		...
		foreach ( $settings as $name => $args ) {
			switch($args['type']) {
				...
				case 'checkbox' :
					echo '
					<label><input type="checkbox" name="XMLSitemapFeed_'.$name.'" id="XMLSitemapFeed_'.$name.'" value="1" '.checked('1', get_option('XMLSitemapFeed_'.$name, $args['default']), false).' '.disabled($disabled, true, false).' /> '.$args['title'].'</label> '.$args['description'];
					break;
				case 'text' :
					...
					break;
				case 'textarea' :
					echo '<label for="XMLSitemapFeed_'.$name.'">'.$args['title'].'</label><br /><textarea name="XMLSitemapFeed_'.$name.'" id="XMLSitemapFeed_'.$name.'" class="'.$args['class'].'"'.disabled($disabled, true, false).' cols="50" rows="5">'.esc_attr( get_option('XMLSitemapFeed_'.$name, $args['default']) ).'</textarea><p class="description">'.$args['description'].'</p>';
					break;
				case 'number' :
	...
	*/
	...
	public function robots($output) {
		...
		if ( !empty($this->sitemaps) )
			...
			echo "\n# Warning: XML Sitemaps are disabled. Please see your sites XML Sitemap settings.";
	...
	public function robots_txt($output) {
		return $output . get_option('XMLSitemapFeed_robots') ;
	}
