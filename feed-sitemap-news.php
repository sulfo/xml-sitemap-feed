...
echo '<?xml version="1.0" encoding="'.get_bloginfo('charset').'"?><?xml-stylesheet type="text/xsl" href="'.get_option('home').'/'.str_replace(ABSPATH,"", XMLSF_PLUGIN_DIR).'/sitemap-news.xsl.php?v='.XMLSF_VERSION.'"?>
...
$maxURLS = 1000;	// maximum number of URLs allowed in any sitemap.
...
?><url><loc><?php echo esc_url( get_permalink() ) ?></loc>

// http://www.google.com/support/news_pub/bin/answer.py?answer=74288
// verplicht:
<news:news>
	<news:publication>
		<news:name>The Example Times</news:name> >> site title or category... ?? 
		<news:language>en</news:language> >> article lang code ...
	</news:publication>
      <news:publication_date>YYYY-MM-DDTuu:mm:ss</news:publication_date>
      <news:title>Companies A, B in Merger Talks</news:title>

// optioneel
      <news:keywords>business, merger, acquisition, A, B</news:keywords> >> tags... and cats ??
	<news:access>Subscription</news:access> >> http://www.google.com/support/news_pub/bin/answer.py?answer=93992
	<news:genres>Blog</news:genres> >> http://www.google.com/support/news_pub/bin/answer.py?answer=93992

</news:news>



<lastmod><?php echo mysql2date('Y-m-d\TH:i:s+00:00', $thispostmodified_gmt, false) ?></lastmod><?php
