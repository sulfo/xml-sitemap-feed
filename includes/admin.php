 ...
 class XMLSF_Admin extends XMLSitemapFeed {
	...
	*/
	...
	public function privacy_settings_section() {
		...
		echo ('1' == get_option('blog_public')) ? __('Note:','xml-sitemap-feed').' '.sprintf(__('XML Sitemaps will be disabled if you set the option %s (above) to %s.','xml-sitemap-feed'),'<strong>'.__('Search Engine Visibility').'</strong>','<strong>'.__('Discourage search engines from indexing this site').'</strong>') : '<span style="color: red" class="error">'.sprintf(__('XML Sitemaps are disabled because you have set the option %s (above) to %s.','xml-sitemap-feed'),'<strong>'.__('Search Engine Visibility').'</strong>','<strong>'.__('Discourage search engines from indexing this site').'</strong>').'</span>';
		echo '</p>
...
<script type="text/javascript">
	...
	public function taxonomies_settings_field() {
		...
		echo '
		<p class="description">' . __('Note:','xml-sitemap-feed').' '.__('Including taxonomy pages is generally not recommended, unless they have intrinsic value on your site. For example when you use category descriptions with information that is not present elsewhere on your site. Or if taxonomy pages list posts with an excerpt that is different from, but complementary to the post content. In these cases you might consider including certain taxonomies.','xml-sitemap-feed') .
		'<br />' .
		__('Note:','xml-sitemap-feed').' '. __('If you want to go further than not including taxonomies: consider disallowing search engine access to them by adding specific robots.txt rules below.','xml-sitemap-feed');
		echo '</p>
		</div>';
	}
