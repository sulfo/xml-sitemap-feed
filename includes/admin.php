	...
	*/
	...
	public function sitemaps_settings_field() {
		...
		echo '<fieldset id="xmlsf_sitemaps"><legend class="screen-reader-text">'.__('XML Sitemaps','xml-sitemap-feed').'</legend>
			<label><input type="checkbox" name="xmlsf_sitemaps[sitemap]" id="xmlsf_sitemaps_index" value="'.XMLSF_NAME.'" '.checked(isset($options['sitemap']), true, false).' '.disabled($disabled, true, false).' /> '.__('XML Sitemap Index','xml-sitemap-feed').'</label>';//xmlsf
		if (isset($options['sitemap']))
			echo '<span class="description"> &nbsp;&ndash;&nbsp; <a href="#xmlsf">'.__('Settings').'</a> &nbsp;&ndash;&nbsp; <a href="'.trailingslashit(get_bloginfo('url')). ( ('' == get_option('permalink_structure')) ? '?feed=sitemap' : $options['sitemap'] ) .'" target="_blank">'.__('View').'</a></span>';
		...
		if (isset($options['sitemap-news']))
			echo '<span class="description"> &nbsp;&ndash;&nbsp; <a href="#xmlnf">'.__('Settings').'</a> &nbsp;&ndash;&nbsp; <a href="'.trailingslashit(get_bloginfo('url')). ( ('' == get_option('permalink_structure')) ? '?feed=sitemap-news' : $options['sitemap-news'] ) .'" target="_blank">'.__('View').'</a></span>';
		else
...
jQuery( document ).ready( function() {
	...
	*/
	...
	public function news_sitemap_settings() {
		echo '<p><a href="https://www.paypal.com/cgi-bin/webscr?cmd=_donations&business=ravanhagen%40gmail%2ecom&item_name=XML%20Sitemap%20Feeds&item_number='.XMLSF_VERSION.'&no_shipping=0&tax=0&charset=UTF%2d8" title="'.sprintf(__('Donate to keep the free %s plugin development & support going!','xml-sitemap-feed'),__('XML Sitemap & Google News Feeds','xml-sitemap-feed')).'"><img src="https://www.paypalobjects.com/en_US/i/btn/btn_donate_LG.gif" style="border:none;float:right;margin:4px 0 0 10px" alt="'.sprintf(__('Donate to keep the free %s plugin development & support going!','xml-sitemap-feed'),__('XML Sitemap & Google News Feeds','xml-sitemap-feed')).'" width="92" height="26" /></a>'.sprintf(__('These settings control the Google News Sitemap generated by the %s plugin.','xml-sitemap-feed'),__('XML Sitemap & Google News Feeds','xml-sitemap-feed')).'</p>';
	}

	public function news_sitemap_settings_field() {
		$options = parent::get_option('news_sitemap');

		echo '
		<fieldset id="xmlsf_taxonomies"><legend class="screen-reader-text">'.__('Google News sitemap settings','xml-sitemap-feed').'</legend>';

		echo '
			<p class="description">'.__('TODO: news publication name (allow tag %category%, default is site title), keywords from category/tags/meta field, post_types (+ exclusion per post or none + allow inclusion per post), limit to category, genre (choose from fixed presets or allow dynamic based on category or better: create new taxonomy for it???)','xml-sitemap-feed');
		echo '</p>
		</fieldset>';
	}
	...
	public function sanitize_news_sitemap_settings($new) {
		return $new;
	}
	...
	*/
	...
	function __construct() {
		...
		if ( isset($sitemaps['sitemap']) || isset($sitemaps['sitemap-news']) ) {
			...
			add_settings_field('xmlsf_ping', __('Ping on Publish','xml-sitemap-feed'), array($this,'ping_settings_field'), 'reading');
		}
		...
		if (isset($sitemaps['sitemap-news'])) {
			...
			// post_types
			register_setting('reading', 'xmlsf_news_sitemap', array($this,'sanitize_news_sitemap_settings') );
			add_settings_field('xmlsf_news_sitemap', __('Google News sitemap settings','xml-sitemap-feed'), array($this,'news_sitemap_settings_field'), 'reading', 'news_sitemap_section');
		}
